...
$sqls = [
    ...
    "CREATE TABLE enroll(
        ...
        lulus boolean, 
        fk_asesor varchar(8), 
        FOREIGN KEY(fk_pendaftar) REFERENCES pendaftar(id_pendaftar) ON DELETE CASCADE,
        FOREIGN KEY(fk_jalur) REFERENCES jalur(id_jalur) ON DELETE CASCADE,
        FOREIGN KEY(fk_prodi_1) REFERENCES prodi(id_prodi) ON DELETE CASCADE,
        FOREIGN KEY(fk_prodi_2) REFERENCES prodi(id_prodi) ON DELETE CASCADE,
        FOREIGN KEY(fk_ujian) REFERENCES ujian(id_ujian) ON DELETE CASCADE,
        FOREIGN KEY(fk_rapor) REFERENCES rapor_utbk(id_rapor) ON DELETE CASCADE,
        FOREIGN KEY(fk_prestasi) REFERENCES prestasi(id_prestasi) ON DELETE CASCADE,
        FOREIGN KEY(fk_asesor) REFERENCES asesor(id_asesor) ON DELETE CASCADE
    )",
    // "drop table jalur"
];
